...
export default function SearchOverlay({ visible, onClose }) {
    ...
    const delayDebounce = setTimeout(async () => {
      ...
      try {
        ...
        if (response.ok) {
          ...
          // Mapping items exactly like your Web implementation
  ...
  const handleItemClick = (restaurantId, name) => {
    if (!restaurantId) return;
    saveSearch(name);
    onClose();
    // Navigates directly to the restaurant screen we created earlier
                ...
                <View style={styles.section}>
                  ...
                  {liveResults.items.map((product) => (
                    <TouchableOpacity 
                      ...
                      onPress={() => handleItemClick(product.restaurantId, product.name)}
                    >
                      <Image 
